    ...
    if (requestUrl.searchParams.get('fontdebug') === '1') {
        const debug = await page.evaluate(() => {
            ...
            return {
                ...
                drawTest: (() => {
                    const raw = getComputedStyle(document.body).getPropertyValue('--base-font')
                    const chartDefault = (window.Chart && window.Chart.defaults && window.Chart.defaults.font) || {}
                    const stacks = [
                        `500 24px ${raw}`,
                        `500 24px ${raw.trim()}`,
                        `500 24px ${chartDefault.family || 'no-chart-default'}`,
                        '500 24px museo-sans, noto-sans-arabic, sans-serif',
                    ]
                    ...
                    stacks.forEach((font, i) => {
                        ctx.font = font
                        applied.push(`${font}  ->  ${ctx.font}`)
                        ctx.fillText('الأمن وسيادة القانون ABC', 10, 30 + i * 40)
                    })
                    window.__applied = applied
